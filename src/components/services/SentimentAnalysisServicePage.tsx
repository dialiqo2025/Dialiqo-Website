...
import { SentimentAnalysisWhyChoose } from "@/components/services/SentimentAnalysisWhyChoose";
import { SentimentAnalysisFaq } from "@/components/services/SentimentAnalysisFaq";
import {
  SENTIMENT_CONTACT,
  SENTIMENT_CTA_BAND,
  SENTIMENT_ELEMENTS,
...
export function SentimentAnalysisServicePage() {
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <h2 id="sa-intro-heading" className={`${typo.sectionTitle} text-white`}>
              {SENTIMENT_INTRO.title}
            </h2>
            {SENTIMENT_INTRO.paragraphs.map((paragraph) => (
      ...
      <section
        ...
        aria-labelledby="sa-elements-heading"
      >
        <div className="max-w-7xl mx-auto">
          <header className="text-center max-w-3xl mx-auto mb-14">
            <h2
              id="sa-elements-heading"
              className={`${typo.sectionTitle} text-slate-900 dark:text-white`}
            >
              {SENTIMENT_ELEMENTS.title}
            </h2>
            <p
              className={`mt-4 ${typo.subtitle} text-slate-600 dark:text-slate-400`}
            >
              {SENTIMENT_ELEMENTS.description}
            </p>
          </header>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {SENTIMENT_ELEMENTS.items.map((item, i) => {
              const Icon = ELEMENT_ICONS[i % ELEMENT_ICONS.length];
              return (
                <article
                  key={item}
      ...
      <section
        ...
        aria-labelledby="sa-types-heading"
      >
        <div className="max-w-7xl mx-auto">
          <header className="text-center max-w-3xl mx-auto mb-14">
            <h2
              ...
              className={`${typo.sectionTitle} text-slate-900 dark:text-white`}
            >
              {SENTIMENT_TYPES.title}
            </h2>
          </header>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {SENTIMENT_TYPES.items.map((item, i) => {
      ...
      >
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <h2 id="sa-cta-heading" className={`${typo.sectionTitle} text-white`}>
            {SENTIMENT_CTA_BAND.title}
          </h2>
          <ServerLinkButton
            href="/contact"
            variant="glow"
