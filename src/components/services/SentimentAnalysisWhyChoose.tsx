...
import { SENTIMENT_WHY } from "@/lib/sentimentAnalysisPageData";
import { typo } from "@/lib/typography";

export function SentimentAnalysisWhyChoose() {
  const [active, setActive] = useState(0);
  const current = SENTIMENT_WHY.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {SENTIMENT_WHY.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
