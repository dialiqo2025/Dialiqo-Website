...
import { HIRE_VOIP_CHOOSE_US } from "@/lib/hireVoipPageData";
import { typo } from "@/lib/typography";

export function HireVoipWhyChoose() {
  const [active, setActive] = useState(0);
  const current = HIRE_VOIP_CHOOSE_US.items[active];

  return (
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {HIRE_VOIP_CHOOSE_US.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
