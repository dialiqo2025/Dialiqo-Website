...
import { VOIP_TESTING_WHY } from "@/lib/voipTestingPageData";
import { typo } from "@/lib/typography";

export function VoipTestingWhyChoose() {
  const [active, setActive] = useState(0);
  const current = VOIP_TESTING_WHY.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {VOIP_TESTING_WHY.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
