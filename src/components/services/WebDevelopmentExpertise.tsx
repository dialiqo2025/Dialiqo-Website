...
import { WEB_DEV_EXPERTISE } from "@/lib/webDevelopmentPageData";
import { typo } from "@/lib/typography";

export function WebDevelopmentExpertise() {
  const [active, setActive] = useState(0);
  const current = WEB_DEV_EXPERTISE.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {WEB_DEV_EXPERTISE.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
