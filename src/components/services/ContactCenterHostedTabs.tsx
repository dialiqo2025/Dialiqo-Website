...
import { CONTACT_CENTER_HOSTED } from "@/lib/contactCenterPageData";
import { typo } from "@/lib/typography";

export function ContactCenterHostedTabs() {
  const [active, setActive] = useState(0);
  const current = CONTACT_CENTER_HOSTED.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {CONTACT_CENTER_HOSTED.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
