...
import { MVNO_BILLING_CUSTOM } from "@/lib/mvnoBillingPageData";
import { typo } from "@/lib/typography";

export function MvnoBillingCustomTabs() {
  const [active, setActive] = useState(0);
  const current = MVNO_BILLING_CUSTOM.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {MVNO_BILLING_CUSTOM.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
