...
import { OPENSIPS_PAIN } from "@/lib/opensipsPageData";
import { typo } from "@/lib/typography";

export function OpensipsPainPoints() {
  const [active, setActive] = useState(0);
  const current = OPENSIPS_PAIN.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {OPENSIPS_PAIN.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
