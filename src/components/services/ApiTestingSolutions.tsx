...
import { API_TESTING_SOLUTIONS } from "@/lib/apiTestingPageData";
import { typo } from "@/lib/typography";

export function ApiTestingSolutions() {
  const [active, setActive] = useState(0);
  const current = API_TESTING_SOLUTIONS.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {API_TESTING_SOLUTIONS.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
