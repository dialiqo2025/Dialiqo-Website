...
import { RealEstateIndustryFaq } from "@/components/industries/RealEstateIndustryFaq";
import {
  REAL_ESTATE_CONTACT,
  REAL_ESTATE_CTA_BAND,
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <section
        ...
        aria-labelledby="re-how-heading"
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <h2 id="re-how-heading" className={`${typo.sectionTitle} text-white`}>
              {REAL_ESTATE_HOW.title}
            </h2>
            {REAL_ESTATE_HOW.paragraphs.map((paragraph) => (
      ...
      <section
        ...
        aria-labelledby="re-why-heading"
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div>
            ...
            >
              {REAL_ESTATE_WHY.title}
            </h2>
            <ul className="mt-8 space-y-4">
              {REAL_ESTATE_WHY.items.map((item) => (
      ...
      <section
        ...
        aria-labelledby="re-features-heading"
      >
        <div className="max-w-7xl mx-auto">
          <header className="text-center max-w-3xl mx-auto mb-14">
            <h2
              id="re-features-heading"
              className={`${typo.sectionTitle} text-slate-900 dark:text-white`}
            >
              {REAL_ESTATE_FEATURES.title}
            </h2>
          </header>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {REAL_ESTATE_FEATURES.items.map((item, i) => {
      ...
      <section
        className="py-20 px-4 sm:px-6 lg:px-8 bg-slate-950 text-white"
        aria-labelledby="re-property-heading"
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <h2
              id="re-property-heading"
              className={`${typo.sectionTitle} text-white`}
            >
              {REAL_ESTATE_PROPERTY_MGMT.title}
            </h2>
            <p className={`mt-5 ${typo.subtitle} text-slate-300`}>
              {REAL_ESTATE_PROPERTY_MGMT.intro}
            </p>
            <ul className="mt-8 space-y-4">
              {REAL_ESTATE_PROPERTY_MGMT.items.map((item) => (
                <li key={item} className="flex items-start gap-3">
                  <CheckCircle2
      ...
      <section
        className="py-20 px-4 sm:px-6 lg:px-8 bg-white dark:bg-slate-900 border-y border-slate-200 dark:border-slate-800"
        aria-labelledby="re-scaling-heading"
      >
        <div className="max-w-3xl mx-auto text-center">
          <h2
            id="re-scaling-heading"
            className={`${typo.sectionTitle} text-slate-900 dark:text-white`}
          >
            {REAL_ESTATE_SCALING.title}
          </h2>
          <p
            className={`mt-5 ${typo.subtitle} text-slate-600 dark:text-slate-400`}
          >
            {REAL_ESTATE_SCALING.paragraph}
          </p>
          <ServerLinkButton
            href="/contact"
            variant="glow"
      ...
      <section
        ...
        aria-labelledby="re-cta-heading"
      >
        <Image
          src={CTA_IMAGE}
          alt=""
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <h2 id="re-cta-heading" className={`${typo.sectionTitle} text-white`}>
            {REAL_ESTATE_CTA_BAND.title}
          </h2>
          <p className="mt-4 text-lg font-semibold text-white">
            {REAL_ESTATE_CTA_BAND.subtitle}
          </p>
          <p className={`mt-4 ${typo.subtitle} text-slate-300`}>
            {REAL_ESTATE_CTA_BAND.paragraph}
          </p>
          <p className="mt-4 text-sm font-semibold text-blue-300">
            {REAL_ESTATE_CTA_BAND.demoLabel}
          </p>
          <ServerLinkButton
            href="/contact"
            variant="glow"
      ...
      <section
        className="py-20 px-4 sm:px-6 lg:px-8 bg-white dark:bg-slate-900 border-y border-slate-200 dark:border-slate-800"
        aria-labelledby="re-faq-heading"
      >
        <div className="max-w-7xl mx-auto">
          <h2
            id="re-faq-heading"
      ...
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <h2
              id="re-contact-heading"
              className={`${typo.sectionTitle} text-slate-900 dark:text-white`}
            >
              {REAL_ESTATE_CONTACT.title}
            </h2>
            <p
              className={`mt-5 ${typo.subtitle} text-slate-600 dark:text-slate-400`}
            >
              {REAL_ESTATE_CONTACT.description}
            </p>
          </div>
          <HomeContactForm />
        </div>
