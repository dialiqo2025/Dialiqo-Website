...
import { KAMAILIO_SOLUTIONS } from "@/lib/kamailioPageData";
import { typo } from "@/lib/typography";

export function KamailioSolutions() {
  const [active, setActive] = useState(0);
  const current = KAMAILIO_SOLUTIONS.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {KAMAILIO_SOLUTIONS.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
