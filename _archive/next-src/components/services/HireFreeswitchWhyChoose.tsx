...
import { HIRE_FREESWITCH_CHOOSE_US } from "@/lib/hireFreeswitchPageData";
import { typo } from "@/lib/typography";

export function HireFreeswitchWhyChoose() {
  const [active, setActive] = useState(0);
  const current = HIRE_FREESWITCH_CHOOSE_US.items[active];

  return (
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {HIRE_FREESWITCH_CHOOSE_US.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
