...
import { QA_AUTO_SOLUTIONS } from "@/lib/qaAutomationPageData";
import { typo } from "@/lib/typography";

export function QaAutomationSolutions() {
  const [active, setActive] = useState(0);
  const current = QA_AUTO_SOLUTIONS.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {QA_AUTO_SOLUTIONS.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
