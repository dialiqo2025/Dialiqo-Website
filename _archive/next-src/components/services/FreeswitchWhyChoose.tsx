...
import { FREESWITCH_WHY } from "@/lib/freeswitchPageData";
import { typo } from "@/lib/typography";

export function FreeswitchWhyChoose() {
  const [active, setActive] = useState(0);
  const current = FREESWITCH_WHY.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {FREESWITCH_WHY.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
