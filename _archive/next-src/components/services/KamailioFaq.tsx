...
import { KAMAILIO_FAQS } from "@/lib/kamailioPageData";
import { typo } from "@/lib/typography";

export function KamailioFaq() {
  const [openIndex, setOpenIndex] = useState<number | null>(0);

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {KAMAILIO_FAQS.map((faq, i) => {
        ...
        return (
          ...
          >
            ...
            <div data-faq-answer className={`${isOpen ? "" : "hidden"} px-5 pb-5 text-sm text-slate-600 dark:text-slate-400 leading-relaxed`}>
                {faq.answer ? <p>{faq.answer}</p> : null}
                {faq.bullets && faq.bullets.length > 0 && (
                  <ul className={`list-disc pl-5 space-y-1 ${faq.answer ? "mt-3" : ""}`}>
