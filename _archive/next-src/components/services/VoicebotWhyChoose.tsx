...
import { VOICEBOT_CHOOSE_US } from "@/lib/voicebotPageData";
import { typo } from "@/lib/typography";

export function VoicebotWhyChoose() {
  const [active, setActive] = useState(0);
  const current = VOICEBOT_CHOOSE_US.items[active];

  return (
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {VOICEBOT_CHOOSE_US.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
