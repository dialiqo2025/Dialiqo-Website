...
import { CHATBOT_PILLARS } from "@/lib/chatbotPageData";
import { typo } from "@/lib/typography";

export function ChatbotPillars() {
  const [active, setActive] = useState(0);
  const current = CHATBOT_PILLARS.items[active];

  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {CHATBOT_PILLARS.items.map((item, i) => (
          <button
            key={item.title}
            type="button"
            onClick={() => setActive(i)}
