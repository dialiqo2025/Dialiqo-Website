...
import { CALL_CENTERS_FAQS } from "@/lib/callCentersIndustryPageData";
import { typo } from "@/lib/typography";

export function CallCentersIndustryFaq() {
  const [openIndex, setOpenIndex] = useState<number | null>(0);

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {CALL_CENTERS_FAQS.map((faq, i) => {
        ...
        return (
          ...
          >
            ...
            <p className={`${isOpen ? "" : "hidden"} px-5 pb-5 text-sm text-slate-600 dark:text-slate-400 leading-relaxed`} data-faq-answer>
                {faq.answer}
              </p>
          </article>
        );
